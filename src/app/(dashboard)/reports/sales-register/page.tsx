...
export default function SalesRegisterPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const params = new URLSearchParams({ companyId: activeCompany!.id, type: "SALES", from, to, limit: "500" });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${vouchers.length} sales invoices`}
        actions={
          <Button variant="outline" size="sm" className="gap-1"
            onClick={() => window.open(`/api/export/excel?type=vouchers&voucherType=SALES&companyId=${activeCompany?.id}&from=${from}&to=${to}`, "_blank")}>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "GST Amount", value: formatCurrency(totalGst), color: "text-orange-600" },
            { label: "Total Sales", value: formatCurrency(totalAmount), color: "text-primary" },
          ].map((s) => (
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="bg-row-alt border-b border-border-subtle">
                  ...
                  <th className="px-3 py-2 text-left text-[11px] font-semibold text-text-muted">Invoice No.</th>
                  <th className="px-3 py-2 text-left text-[11px] font-semibold text-text-muted">Party</th>
              ...
              <tbody>
                {vouchers.length === 0 ? (
                  <tr><td colSpan={9} className="px-3 py-12 text-center text-text-muted">
                    ...
                    <p>No sales invoices found for this period</p>
                  </td></tr>
                ) : vouchers.map((v, i) => (
                  <tr key={v.id} className={`border-b border-border-subtle ${i % 2 === 0 ? "bg-white" : "bg-row-alt"} hover:bg-primary/5`}>
                    ...
                    <td className="px-3 py-2">
                      <Link href={`/vouchers/SALES/${v.id}`} className="text-primary hover:underline font-medium">
                        {v.number}
                      </Link>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <Badge className={`text-[9px] ${v.status === "CANCELLED" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>
                        {v.status ?? "POSTED"}
                      </Badge>
                    </td>
                  </tr>
