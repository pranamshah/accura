...
export default function PayablesPage() {
  ...
  const { data, isLoading } = useQuery<{ rows: Array<{ ledgerId: string; ledgerName: string; totalDue: number; bills: Array<{ billRef: string; billDate: string; amount: number; ageDays: number }> }>; total: number }>({
    queryKey: ['outstanding-payables', activeCompany?.id, asOf],
    queryFn: async () => {
      if (!activeCompany) return { rows: [], total: 0 };
      const r = await fetch(`/api/reports/outstanding?companyId=${activeCompany.id}&type=payable&asOf=${asOf}`);
      return r.json();
    },
  ...
  return (
    <div className="report-screen">
      <div className="report-header">
        <div className="report-title">OUTSTANDING PAYABLES</div>
      ...
      {isLoading ? <div style={{ color: '#a0a0a0', padding: 12 }}>Loading...</div> : (
        <table className="report-table">
          ...
          <tbody>
            {(data?.rows ?? []).map((row) => (
              <>
                <tr key={row.ledgerId} className="group-row">
                  ...
                  <td className="report-amount" style={{ color: '#FF4444' }}>{formatCurrency(row.totalDue)}</td>
                </tr>
                {row.bills.map((b, bi) => (
                  <tr key={bi}>
                    ...
                    <td className="report-amount" style={{ color: b.ageDays > 90 ? '#FF4444' : b.ageDays > 30 ? '#FFD700' : '#a0a0a0' }}>
                      {b.ageDays}
                    </td>
            ...
            {(data?.rows ?? []).length === 0 && (
              <tr><td colSpan={5} style={{ textAlign: 'center', color: '#a0a0a0', padding: 20 }}>No outstanding payables</td></tr>
            )}
          </tbody>
          {(data?.rows ?? []).length > 0 && (
            <tfoot>
              <tr className="total-row">
                ...
                <td className="report-amount" style={{ color: '#FF4444' }}>{formatCurrency(data?.total ?? 0)}</td>
              </tr>
            </tfoot>
          )}
        </table>
